...
<div class="container">
    <div class="col-md-6 col-md-offset-3">
        <form action="<?= $_SERVER['REQUEST_URI'] ?>" method="post" enctype="multipart/form-data">
            ...
            ?>
            <div class="form-group"><label for="date">Choose date</label><input id="date" name="date"
                                                                                value="<?= $chosen_date ?>"></div>
        ...
        //this function checks whether value exists in an array
        ...
        if (in_multidimensional_array($team_abbreviation, $array, "4")) {
            ...
            foreach ($scoreboard as $rowSet) {
                ...
                $result = array_merge($array[$rowCount], $array[$rowCount + 1]);
                ...
                if ($rowCount++ % 2 == 1 && $result[4] == $team_abbreviation || $result[4 + 28] == $team_abbreviation) {
                    ...
                    echo '<tr><td>' . $result[5] . '</td><td>' . $result[7] . '</td><td>' . $result[8] . '</td><td>' . $result[9] . '</td><td>' . $result[10] . '</td>' . ($result[11] ? '<td>' . $result[11] . '</td>' : '') . ($result[12] ? '<td>' . $result[12] . '</td>' : '') . ($result[13] ? '<td>' . $result[13] . '</td>' : '') . ($result[14] ? '<td>' . $result[14] . '</td>' : '') . ($result[15] ? '<td>' . $result[15] . '</td>' : '') . ($result[16] ? '<td>' . $result[16] . '</td>' : '') . '<td>' . $result[21] . '</td></tr>';
                    echo '<tr><td>' . $result[5 + 28] . '</td><td>' . $result[7 + 28] . '</td><td>' . $result[8 + 28] . '</td><td>' . $result[9 + 28] . '</td><td>' . $result[10 + 28] . '</td>' . ($result[11 + 28] ? '<td>' . $result[11 + 28] . '</td>' : '') . ($result[12 + 28] ? '<td>' . $result[12 + 28] . '</td>' : '') . ($result[13 + 28] ? '<td>' . $result[13 + 28] . '</td>' : '') . ($result[14 + 28] ? '<td>' . $result[14 + 28] . '</td>' : '') . ($result[15 + 28] ? '<td>' . $result[15 + 28] . '</td>' : '') . ($result[16 + 28] ? '<td>' . $result[16 + 28] . '</td>' : '') . '<td>' . $result[21 + 28] . '</td></tr>';
                    echo '</table>';
                }
                $rowCount++;
            }
